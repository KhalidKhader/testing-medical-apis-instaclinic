...
======================================
Transcribes medical conversations using Whisper v3 Turbo with NeMo/PyAnnotate diarization.

- Uses HuggingFace's Whisper v3 Turbo model for high-quality, faster transcription
...
Usage:
    python transcribe_medical_whisper_v3_turbo.py audio_file [output_file]
"""
...
def load_whisper_model():
    """Load and cache the Whisper v3 Turbo model from HuggingFace."""
    try:
        from transformers import AutoProcessor, AutoModelForSpeechSeq2Seq
        
        logger.info("Loading Whisper v3 Turbo model from HuggingFace...")
        
        # Check if CUDA is available and set device accordingly
        device = "cuda:0" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu"
        torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32
        
        # Load the model and processor with HF token
        ...
        pipe = pipeline(
            ...
            generate_kwargs={"condition_on_prev_tokens": False}  # Explicitly set for compatibility
        )
        
        logger.info(f"Successfully loaded Whisper v3 Turbo model (on {device})")
        return pipe
...
def transcribe_audio_with_whisper_turbo(audio_path, model=None, task="transcribe"):
    ...
    try:
        ...
        if model is None:
            from transformers import pipeline
            logging.info("Loading Whisper v3 Turbo model...")
            
            # Check if CUDA is available and set device accordingly
            device = "cuda:0" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu"
            torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32
            
            # Use transformers pipeline for Whisper v3 Turbo
            pipe = pipeline(
                "automatic-speech-recognition",
        ...
        # Set up the parameters for the pipeline
        generate_kwargs = {
            "max_new_tokens": 300,
            "return_timestamps": True,
            "task": task,
            # Remove condition_on_prev_tokens as it's not supported in the current version
        }
...
def process_all_directories():
    ...
    for category in categories:
        ...
        for item in os.listdir(category_dir):
            if "whisper-v3-turbo" in item and os.path.isdir(os.path.join(category_dir, item)):
                language_dir = os.path.join(category_dir, item)
                audio_dir = os.path.join(language_dir, "audio")
                
                if os.path.isdir(audio_dir):
                    logger.info(f"Processing {category}/{item}/audio")
...
def transcribe_audio_file(audio_path, model=None, output_path=None):
    """
    Process a single audio file with Whisper v3 Turbo and diarization.
    ...
    try:
        ...
        if model is None:
            logger.info("Loading Whisper v3 Turbo model...")
            model = load_whisper_model()
            if not model:
                logger.error("Failed to load Whisper model")
                return None
        
        # Step 1: Transcribe the audio with Whisper v3 Turbo model
        logger.info("Starting transcription with Whisper v3 Turbo model...")
        transcription = transcribe_audio_with_whisper_turbo(audio_path, model, task="transcribe")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python transcribe_medical_whisper_v3_turbo.py [audio_file|directory] [output_file]")
        print("Examples:")
        print("  python transcribe_medical_whisper_v3_turbo.py audio.wav")
        print("  python transcribe_medical_whisper_v3_turbo.py audio.wav output.json")
        print("  python transcribe_medical_whisper_v3_turbo.py directory_with_audio_files")
        print("  python transcribe_medical_whisper_v3_turbo.py --process-all")
        return
