...
def setup_argparse():
    ...
    parser.add_argument('--model-name', type=str, default='transkriptor', 
                       help='Model name to use in output files')
    parser.add_argument('--eval-transkriptor', action='store_true', default=True,
                      help='Evaluate Transkriptor transcripts')
...
def extract_speaker_turns(data, source_type='original'):
    ...
    if source_type == 'original':
        # Original JSON format from med-data/*/json
        ...
        if isinstance(data, dict):
            if 'transcript' in data:
                # Format with 'transcript' array
                turns = []
                for turn in data['transcript']:
                    # Keep the original speaker ID for now - we'll map it during evaluation
                    turns.append({
                        'speaker': turn['speaker'],
                        'text': turn['text'],
                        'timestamp': turn.get('timestamp', '')  # Include timestamp if available
                    })
                return turns
            elif 'conversation' in data:
                # Format with 'conversation' array (the same as original)
                return data['conversation']
...
def calculate_speaker_accuracy(original_turns, transcript_turns):
    ...
    This method accounts for the SPK_1/SPK_2 format in Transkriptor outputs where the speaker
    numbering may not align with doctor/patient roles in original data.
    """
    ...
    # Extract speakers from original (expected to be 'doctor' or 'patient')
    original_speakers = [turn['speaker'].lower() for turn in original_turns]
    
    # Extract speakers from transcript (expected to be SPK_1, SPK_2, speaker_1, etc.)
    transcript_speakers = [turn['speaker'] for turn in transcript_turns]
    
    # Determine the most likely mapping of SPK_* to doctor/patient
    # Strategy: try both possible mappings and see which gives higher accuracy
    
    # Create speaker id set from transcript
    unique_transcript_speakers = sorted(list(set(transcript_speakers)))
    
    if len(unique_transcript_speakers) < 2:
        # If only one speaker was detected, accuracy is at most 50%
        return 0.5 * turn_structure_similarity
    
    # For Transkriptor: try both possible mappings
    # Mapping 1: SPK_1/speaker_1 = doctor, SPK_2/speaker_2 = patient
    mapping1 = {
        unique_transcript_speakers[0]: "doctor",
        unique_transcript_speakers[1]: "patient"
    }
    
    # Mapping 2: SPK_1/speaker_1 = patient, SPK_2/speaker_2 = doctor
    mapping2 = {
        unique_transcript_speakers[0]: "patient",
        unique_transcript_speakers[1]: "doctor"
    }
    
    # Apply both mappings and see which produces higher accuracy
    correct_speakers1 = 0
    correct_speakers2 = 0
    ...
    for i in range(min_length):
        ...
        transcript_speaker = transcript_speakers[i]
        
        # Check accuracy with mapping 1
        if mapping1.get(transcript_speaker, "unknown").lower() == original_speaker:
            correct_speakers1 += 1
        
        # Check accuracy with mapping 2
        if mapping2.get(transcript_speaker, "unknown").lower() == original_speaker:
            correct_speakers2 += 1
    
    # Use the mapping that gives higher accuracy
    position_accuracy = max(correct_speakers1, correct_speakers2) / min_length if min_length > 0 else 0.0
...
def calculate_medical_term_accuracy(original_text, transcript_text, language='en'):
    ...
    for orig_term in remaining_original:
        for trans_term in list(remaining_transcript):  # Create a copy to safely modify during iteration
            ...
            if norm_dist < 0.3:
                near_matches += 1
                near_match_pairs.append((orig_term, trans_term))
...
def evaluate_file_pair(original_path, transcript_path, language):
    """Evaluate a single pair of original and transcript files from Transkriptor"""
    try:
        # Load files
...
def process_file_pairs(base_dir, model_name='transkriptor'):
    """Process all file pairs for evaluation with Transkriptor-generated transcripts"""
    results = []
    ...
    required_columns = ['file', 'specialty', 'language_code', 'language', 'visit_type', 'speaker_accuracy', 
                       ...
                       'exact_matches', 'near_matches', 'original_doctor_terms', 'original_patient_terms',
                       'transcript_doctor_terms', 'transcript_patient_terms']
...
def main():
    ...
    # Check if we need to evaluate Transkriptor transcripts
    if args.eval_transkriptor:
        print(f"Processing file pairs from directory: {args.data_dir}")
        
        # Path to specific Without-noise Azure for French-Nova-3-medical for English directory
        specific_path = "results/all-data/Without-noise-Azure for French-Nova-3-medical- for English"
        
        # Check if the specific path exists
        if os.path.exists(specific_path):
            print(f"Found specific path: {specific_path}")
            results_df = process_file_pairs(specific_path, args.model_name)
        else:
            # Fallback to the general data directory
            print(f"Using general data directory: {args.data_dir}")
        ...
        results_path = os.path.join(output_dir, 'medical_term_eval_results.csv')
